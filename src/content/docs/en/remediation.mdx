...
1. Once the document is loaded, you will be presented with the first two pages of the document, or the entire document if you have page credit.
...
1. Below the document content, you will find a "Remediate Full Document" button. Activating this button will allow you to select a conversion format via a menu, as well as customize conversion settings that depend on your chosen format. Regardless of conversion format, the basic conversion settings allow you to determine whether a document should have image descriptions generated in your currently chosen application language, or whether or not to have math content converted into accessible MathML. The available conversion formats include:
    ...
    * Braille
        ...
        * Additional settings include:
            * Braille cells per line (40 by default)
            * Whether to split Braille into pages
                * Number of lines per page (only shows if page splitting is enabled)
...
1. Once you have selected your format and configured any additional settings for the chosen format, do one of the following:
    1. Activate the "Next: Pricing Options" button to begin the payment process.
        * You can choose to purchase pages À la carte, or buy an annual subscription. The [Plans and Purchasing](../plans) section will cover the plans in more detail.
...
1. from within the customization screen, check the "Process mathematical notation (beta)" checkbox.
...
Once logged into your Scribe account, a "Previously Scribed Documents" section will display the documents that you have already remediated. Clicking on one of them will pull up its preview along with any existing conversion downloads. You may also move the document to the trash via the "Move to Trash" button.

#### Trash Section

The "Previously Scribed Documents" section features a "Trash" link below the table of documents. This page allows you to access any documents you have placed in the trash, as well as view the date at which they are scheduled to be permanently deleted. Below each document, you will encounter two buttons:
* Recover: Removes the document from the trash, restoring it back to the main document library.
* Delete Permanently: Permanently deletes the document from the account.
	* This function requires user confirmation.

Any documents that are placed in the trash are retained for a maximum of seven days before being permanently deleted. If your document retention period is set to a shorter duration, the trash will delete your documents according to the retention preference instead.